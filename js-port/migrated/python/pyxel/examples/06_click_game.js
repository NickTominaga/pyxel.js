...
class Bubble {
    constructor() {
        ...
        this.pos = Vec2(;
            // TODO(python): pyxel.rndf(self.r, SCREEN_WIDTH - self.r),
            // TODO(python): pyxel.rndf(self.r, SCREEN_HEIGHT - self.r),
        // TODO(python): )
        this.vel = Vec2(;
            // TODO(python): pyxel.rndf(-MAX_BUBBLE_SPEED, MAX_BUBBLE_SPEED),
            // TODO(python): pyxel.rndf(-MAX_BUBBLE_SPEED, MAX_BUBBLE_SPEED),
        // TODO(python): )
    ...
    update() {
        ...
        if (this.vel.x < 0 && this.pos.x < this.r) {
            this.vel.x *= -1;
        }
        if (this.vel.x > 0 && this.pos.x > SCREEN_WIDTH - this.r) {
            this.vel.x *= -1;
        }
        if (this.vel.y < 0 && this.pos.y < this.r) {
            this.vel.y *= -1;
        }
        if (this.vel.y > 0 && this.pos.y > SCREEN_HEIGHT - this.r) {
            this.vel.y *= -1;
...
class App {
        ...
        if (pyxel.btnp(pyxel.MOUSE_BUTTON_LEFT)) {
            for (const i of range(num_bubbles)) {
                ...
                if (dx * dx + dy * dy < bubble.r * bubble.r) {
                    ...
                    // TODO(python): del self.bubbles[i]
                    break;
        ...
        for (const i of range(num_bubbles - 1, -1, -1)) {
            ...
            for (const j of range(i - 1, -1, -1)) {
                ...
                if (dx * dx + dy * dy < total_r * total_r) {
                    ...
                    // TODO(python): del self.bubbles[i]
                    // TODO(python): del self.bubbles[j]
                    num_bubbles -= 1;
                    break;
    ...
    draw() {
        ...
        if (! this.is_exploded && pyxel.frame_count % 20 < 10) {
