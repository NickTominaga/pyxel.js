...
function cli() {
    // TODO(python): commands = [
        // TODO(python): (["run", "PYTHON_SCRIPT_FILE(.py)"], run_python_script),
        // TODO(python): (
            // TODO(python): ["watch", "WATCH_DIR", "PYTHON_SCRIPT_FILE(.py)"],
            // TODO(python): watch_and_run_python_script,
        // TODO(python): ),
        // TODO(python): (["play", f"PYXEL_APP_FILE({pyxel.APP_FILE_EXTENSION})"], play_pyxel_app),
        // TODO(python): (
            // TODO(python): ["edit", f"[PYXEL_RESOURCE_FILE({pyxel.RESOURCE_FILE_EXTENSION})]"],
            // TODO(python): edit_pyxel_resource,
        // TODO(python): ),
        // TODO(python): (["package", "APP_DIR", "STARTUP_SCRIPT_FILE(.py)"], package_pyxel_app),
        // TODO(python): (
            // TODO(python): ["app2exe", f"PYXEL_APP_FILE({pyxel.APP_FILE_EXTENSION})"],
            // TODO(python): create_executable_from_pyxel_app,
        // TODO(python): ),
        // TODO(python): (
            // TODO(python): ["app2html", f"PYXEL_APP_FILE({pyxel.APP_FILE_EXTENSION})"],
            // TODO(python): create_html_from_pyxel_app,
        // TODO(python): ),
        // TODO(python): (["copy_examples"], copy_pyxel_examples),
    // TODO(python): ]

    function printUsage(command_name) {
        // TODO(python): print("usage:")
        for (const command of commands) {
            if (command_name === null || command[0] == command_name) {
                print(`    pyxel {' '.join(command[0])}`);
            }
        }
    ...
    if (num_args <= 1) {
        print(`Pyxel {pyxel.VERSION}, a retro game engine for Python`);
        print_usage();
        return;
    ...
    for (const command of commands) {
        ...
        if (min_args <= num_args <= max_args) {
            // TODO(python): command[1](*sys.argv[2:])
            return;
        }
        else {
    ...
    // TODO(python): print(f"invalid command: '{sys.argv[1]}'")
    print_usage();
    sys.exit(1);
...
function CheckNewerVersion() {
    ...
    // TODO(python): try:
        // TODO(python): with urllib.request.urlopen(req, timeout=3) as res:
    ...
    // TODO(python): except urllib.error.URLError:
        return;
    if (! latest_version) {
        return;
    ...
    if (parse_version(latest_version) > parse_version(pyxel.VERSION)) {
        print(`A new version, Pyxel {latest_version}, is available.`);
...
function CompleteExtension(filename, command, valid_ext) {
    ...
    if (! file_ext) {
        ...
        print(`'{command}' command only accepts {valid_ext} files`);
        sys.exit(1);
    }
    return filename;
...
function CheckFileExists(filename) {
    if (! os.path.isfile(filename)) {
        // TODO(python): print(f"no such file: '{filename}'")
        sys.exit(1);
...
function CheckDirExists(dirname) {
    if (! os.path.isdir(dirname)) {
        // TODO(python): print(f"no such directory: '{dirname}'")
        sys.exit(1);
...
function CheckFileUnderDir(filename, dirname) {
    if (os.path.relpath(filename, dirname).startswith("..")) {
        print("specified file is ! under the directory");
        sys.exit(1);
...
function CreateAppDir() {
    ...
    for (const path of glob.glob(os.path.join(play_dir, "*"))) {
        ...
        // TODO(python): except ValueError:
            shutil.rmtree(path);

    }
    let app_dir = os.path.join(play_dir, `{os.getpid()}_{uuid.uuid4()}`);
...
function CreateWatchStateFile() {
    ...
    // TODO(python): with open(watch_state_file, "w") as f:
        f.write("");
...
function ExtractPyxelApp(pyxel_app_file) {
    ...
    for (const setting_file of glob.glob(pattern)) {
        // TODO(python): with open(setting_file, "r") as f:
...
function MakeMetadataComment(startup_script_file) {
    ...
    // TODO(python): with open(startup_script_file, "r", encoding="utf8") as f:
        for (const line of f) {
    ...
    for (const key of METADATA_FIELDS) {
        if (key in metadata) {
            ...
            // TODO(python): metadata_comment += f"{key.ljust(max_key_len)} : {value}\n"
        }
    }
...
function watchAndRunPythonScript(watch_dir, python_script_file) {
    ...
    // TODO(python): try:
        print(`start watching '{watch_dir}' (Ctrl+C to stop)`);
        ...
        while (true) {
            ...
            if (cur_time - last_time >= 10) {
                ...
                print(`watching '{watch_dir}' (Ctrl+C to stop)`);
            ...
            if (;
                ...
                || worker.exitcode == pyxel.WATCH_RESET_EXIT_CODE;
            // TODO(python): ):
                print(`rerun {python_script_file}`);
                if (worker.is_alive()) {
                    worker.terminate();
            }
                let worker = _run_python_script_in_separate_process(python_script_file);
    ...
    // TODO(python): except KeyboardInterrupt:
        print("\r", end="");
...
function getPyxelAppMetadata(pyxel_app_file) {
    ...
    for (const line of comment.splitlines()) {
        ...
        if (":" in line) {
            // TODO(python): key, value = line.split(":", 1)
...
function playPyxelApp(pyxel_app_file) {
    ...
    // TODO(python): print(f"file not found: '{pyxel.APP_STARTUP_SCRIPT_FILE}'")
    sys.exit(1);
...
function packagePyxelApp(app_dir, startup_script_file) {
    ...
    // TODO(python): with open(setting_file, "w") as f:
    ...
    // TODO(python): with zipfile.ZipFile(
        // TODO(python): pyxel_app_file,
        // TODO(python): "w",
        // TODO(python): compression=zipfile.ZIP_DEFLATED,
    // TODO(python): ) as zf:
...
function createExecutableFromPyxelApp(pyxel_app_file) {
    ...
    // TODO(python): with open(startup_script_file, "w") as f:
        f.write(;
            "import os, pyxel.cli; pyxel.cli.play_pyxel_app(";
            `os.path.join(os.path.dirname(__file__), '{pyxel_app_name}{pyxel.APP_FILE_EXTENSION}'))`;
        // TODO(python): )

    let cp = subprocess.run("pyinstaller -h", capture_output=true, shell=true);
    if (cp.returncode != 0) {
        print("Pyinstaller is ! found. Please install it.");
        sys.exit(1);

    }
    let command = `"{sys.executable}" -m PyInstaller --windowed --onedir --distpath . `;
    command += `--add-data "{pyxel_app_file}"{os.pathsep}. `;
    let modules = pyxel.utils.list_imported_modules(_extract_pyxel_app(pyxel_app_file))[;
        "system";
    // TODO(python): ]
    command += "".join([`--hidden-import {module} ` for module in modules]);
...
function createHtmlFromPyxelApp(pyxel_app_file) {
    ...
    let base64_string = "";
    // TODO(python): with open(pyxel_app_file, "rb") as f:
        let base64_string = base64.b64encode(f.read()).decode();

    let pyxel_app_name = os.path.splitext(os.path.basename(pyxel_app_file))[0];
    // TODO(python): with open(pyxel_app_name + ".html", "w") as f:
        f.write(;
            "<!doctype html>\n";
            // TODO(python): f'<script src="https://cdn.jsdelivr.net/gh/kitao/pyxel@{pyxel.VERSION}/wasm/pyxel.js">'
            "</script>\n";
            "<script>\n";
            // TODO(python): f'launchPyxel({{ command: "play", name: "{pyxel_app_name}{pyxel.APP_FILE_EXTENSION}", '
            // TODO(python): f'gamepad: "enabled", base64: "{base64_string}" }});\n'
            "</script>\n";
        // TODO(python): )
